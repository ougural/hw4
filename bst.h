...
protected:
    ...
    int maxHeight(Node<Key, Value> *r) const;
		void clearHelper(Node<Key, Value>* curr);
		void removeHelper(Node<Key, Value>* temp);
		bool isBalancedHelper(Node<Key, Value>* temp) const;
...
{		
	...
	bool isRoot = temp->getParent() ? false : true;
	if(!isRoot && temp->getParent()->getLeft() == temp){
		isLeftChild = true;
	}
    // if leaf node
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    if(temp == nullptr){
		...
		// if temp has no left child
			...
			// go up till temp is not a right child, then if temp is a left child return parent, else null
			bool check = true;
			while(check){
...
BinarySearchTree<Key, Value>::successor(Node<Key, Value>* current)
{
    ...
    if(temp == nullptr){
		...
		// if temp has no right child
			...
			// go up till temp is not a left child, then if temp is a right child return parent, else null
			bool check = true;
			while(check){
				if(temp->getParent() == nullptr){
					return nullptr;
...
bool BinarySearchTree<Key, Value>::isBalancedHelper(Node<Key, Value>* temp) const
{
	...
	int heightLeft = maxHeight(temp->getLeft());
	int heightRight = maxHeight(temp->getRight());
	if(abs(heightLeft - heightRight) <= 1 && isBalancedHelper(temp->getLeft()) && isBalancedHelper(temp->getRight())){
		return 1;
	}
	else{
		return 0;
...
// helper function for isBalanced
template<typename Key, typename Value>
int BinarySearchTree<Key, Value>::maxHeight(Node<Key, Value> *curr) const
{
    if(curr == nullptr){
      return 0;
    }
    else{
			return std::max(maxHeight(curr->getRight()), maxHeight(curr->getLeft())) + 1;
		}
}
